...
import { EntityID, Networked, Owned, MaterialTag, App } from "hubs";
import { GLTFMaterial } from "../nodes/entity-nodes";
import { BehaviorGraph, NetworkedMaterial } from "../components";
import { MaterialFlags } from "../inflators/networked-material";
import { Texture } from "three";
import { engines } from "./behavior-graphs";
...
export function material2NetworkedMaterial(eid: EntityID, mat: GLTFMaterial) {
  ...
  NetworkedMaterial.alphaMapNid[eid] = mat.alphaMap
    ? Networked.id[mat.alphaMap.eid!]
    : 0;
  if (mat.toneMapped) {
  ...
  NetworkedMaterial.emissiveMapNid[eid] = mat.emissiveMap
    ? Networked.id[mat.emissiveMap.eid!]
    : 0;
  NetworkedMaterial.emissiveIntensity[eid] = mat.emissiveIntensity;
  NetworkedMaterial.roughnessMapNid[eid] = mat.roughnessMap
    ? Networked.id[mat.roughnessMap.eid!]
    : 0;
  NetworkedMaterial.roughness[eid] = mat.roughness;
  NetworkedMaterial.metalnessMapNid[eid] = mat.metalnessMap
    ? Networked.id[mat.metalnessMap.eid!]
    : 0;
  NetworkedMaterial.metalness[eid] = mat.metalness;
  NetworkedMaterial.lightMapNid[eid] = mat.lightMap
    ? Networked.id[mat.lightMap.eid!]
    : 0;
  NetworkedMaterial.lightMapIntensity[eid] = mat.lightMapIntensity;
  NetworkedMaterial.aoMapNid[eid] = mat.aoMap
    ? Networked.id[mat.aoMap.eid!]
    : 0;
  NetworkedMaterial.aoMapIntensity[eid] = mat.aoMapIntensity;
  NetworkedMaterial.normalMapNid[eid] = mat.normalMap
    ? Networked.id[mat.normalMap.eid!]
    : 0;
  if (mat.wireframe) {
...
const networkedMaterialQuery = defineQuery([
  MaterialTag,
  Networked,
  NetworkedMaterial,
]);
...
export function materialSystem(app: App) {
    ...
    if (material) {
      if (!hasComponent(world, Owned, eid)) {
        ...
        if (
          ...
          NetworkedMaterial.emissiveIntensity[eid]
        ) {
        ...
        if (
          ...
          NetworkedMaterial.lightMapIntensity[eid]
        ) {
        ...
        const isWireframe =
          NetworkedMaterial.flags[eid] & MaterialFlags.WIREFRAME ? true : false;
        ...
        const isFog =
          NetworkedMaterial.flags[eid] & MaterialFlags.FOG ? true : false;
  ...
  behaviorGraphExitQuery(world).forEach(function (eid: EntityID) {
